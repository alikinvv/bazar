...
var mainSliderLoop = $('.main .swiper-slide').length > 1 ? true : false;
...
$(window).scroll(function (event) {
  ...
  if (st > sidebarOffset && $(window).width() > 1023 && $('.order__sidebar').length > 0) {
    ...
    // }
    if (st < $('.subscribe').offset().top - $('.order__sidebar').outerHeight()) {
      $('.order__sidebar').css('left', $('.order__sidebar').offset().left).css('top', 0).css('bottom', 'initial').css('right', 'initial').css('position', 'fixed').removeClass('bottom');
...
var time = document.querySelectorAll('.time-mask');
